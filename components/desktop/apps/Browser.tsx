...
const SEARCH_RESULTS = [
  { name: "E-Commerce", url: "https://your-shop-demo.vercel.app", desc: "Full-stack React & Node shop teszt test test test test test test test", icon: "🛍️" },
  { name: "Chat App", url: "https://your-chat-app.vercel.app", desc: "Socket.io real-time chat platform", icon: "💬" },
  { name: "SaaS Dashboard", url: "https://your-dashboard.vercel.app", desc: "Next.js analytics with charting", icon: "📊" },
  { name: "3D Portfolio Extra", url: "https://your-threejs-build.github.io", desc: "Interactive Three.js graphics sandbox", icon: "🌐" },
  { name: "3D Portfolio Extra1", url: "https://your-threejs-build.github.io", desc: "Interactive Three.js graphics sandbox", icon: "🌐" },
  { name: "3D Portfolio Extra2", url: "https://your-threejs-build.github.io", desc: "Interactive Three.js graphics sandbox", icon: "🌐" },
  { name: "Portfolio", url: "https://nonoro-official.github.io/portfolio/", desc: "Original portfolio website", icon: "🌐" },
];
    ...
    <div className="w-full h-full flex flex-col bg-[#FDFBF7] text-zinc-800 font-sans select-text">
      ...
      <div className="flex items-center gap-3 px-4 py-2 bg-[#EADBB6]/40 border-b border-zinc-200/50 shrink-0">
        <div className="flex items-center gap-1.5 text-zinc-600">
          ...
          <button onClick={goHome} className="p-1 hover:bg-zinc-200/50 rounded transition">
            <Home className="size-4" />
          </button>
          <button onClick={refreshPage} className="p-1 hover:bg-zinc-200/50 rounded transition">
        ...
        <form 
          onSubmit={(e) => {
            e.preventDefault();
            if (inputUrl.trim()) navigateTo(inputUrl.startsWith("http") ? inputUrl : `https://${inputUrl}`);
          }}
          ...
          <div className="max-w-2xl mx-auto px-6 py-4 flex flex-col">
            ...
            <div className="w-full gap-4">
              {SEARCH_RESULTS.map((site) => (
                ...
                >
                  <span className="text-2xl p-2 shadow-inner group-hover:scale-105">{site.icon}</span>
                  <div className="truncate">
                    <h3 className="font-semibold text-zinc-800 text-sm group-hover:text-amber-700 ">{site.name}</h3>
                    <p className="text-xs text-zinc-400 truncate mt-1">{site.desc}</p>
                  </div>
                </button>
              ))}
            </div>
